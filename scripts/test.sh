...
TEST_DB="$test_dir/jobs.db" timeout 60 "$JAVA" "${JAVA_OPTS[@]}" -cp "$test_dir/classes:$SQLITE_JDBC" Default.Main

wait_marker() {
    local pid="$1" log="$2" marker="$3"
    local deadline=$((SECONDS + 20))
    until rg -q --crlf -- "$marker" "$log"; do
...
TEST_DB="$test_dir/crash.db" KOF_TEST_ACTION=hold "$JAVA" "${JAVA_OPTS[@]}" -cp "$cp_runtime" Default.Main > "$test_dir/hold.log" 2>&1 &
child_pids+=("$!")
...
recovery="$(TEST_DB="$test_dir/crash.db" KOF_TEST_ACTION=recover timeout 20 "$JAVA" "${JAVA_OPTS[@]}" -cp "$cp_runtime" Default.Main)"
test "$recovery" = "RECOVERED $job_id"
echo 'PASS SIGKILL depois do claim e recuperação em novo processo'

TEST_DB="$test_dir/rollback.db" KOF_TEST_ACTION=uncommitted "$JAVA" "${JAVA_OPTS[@]}" -cp "$cp_runtime" Default.Main > "$test_dir/uncommitted.log" 2>&1 &
child_pids+=("$!")
...
for index in {1..8}; do
    TEST_DB="$test_dir/race.db" KOF_TEST_ACTION=race timeout 20 "$JAVA" "${JAVA_OPTS[@]}" -cp "$cp_runtime" Default.Main > "$test_dir/race-$index.log" 2>&1 &
    child_pids+=("$!")
done
for pid in "${child_pids[@]}"; do
    wait "$pid"
...
child_pids=()
test "$(rg -l --crlf '^WON$' "$test_dir"/race-*.log | wc -l)" -eq 1
test "$(rg -l --crlf '^EMPTY$' "$test_dir"/race-*.log | wc -l)" -eq 7
